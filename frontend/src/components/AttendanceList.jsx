                      ...
                      <tr className={`hover:bg-slate-50/50 transition-colors group ${isExpanded ? 'bg-slate-50/80' : ''}`}>
                        ...
                        <td className="px-8 py-5 text-right">
                          <div className="flex items-center justify-end space-x-2">
                            <button 
                              type="button"
                              onClick={() => onAttendance(student._id, 'bor')}
                            ...
                            <button 
                              type="button"
                              onClick={() => onAttendance(student._id, "yo'q")}
                              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
        ...
        <div className="md:hidden divide-y divide-slate-100">
          {filteredStudents.length === 0 ? (
            ...
            filteredStudents.map((student) => {
              ...
              return (
                <div key={student._id} className="p-4 space-y-4">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <span className="text-[10px] font-bold text-indigo-500 bg-indigo-50 px-1.5 py-0.5 rounded uppercase">
                          {groups.find(g => g._id === student.groupId)?.name || 'Guruhsiz'}
                        </span>
                      </div>
                    </div>
                    <button 
                      onClick={() => toggleExpand(student._id)}
                      className={`p-2 rounded-lg ${isExpanded ? 'bg-slate-100 text-slate-800' : 'text-slate-400'}`}
                    >
...
const AttendanceHistory = ({ student }) => (
  <div className="bg-white md:bg-transparent rounded-2xl border border-slate-100 md:border-0 p-4 md:p-6 shadow-sm md:shadow-none">
